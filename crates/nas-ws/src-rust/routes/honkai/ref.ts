...
function pityRate(
  ...
  return (pity) =>
    pity < pityStart
      ? baseRate
      : baseRate + baseRate * 10 * (pity - pityStart + 1);
}
    ...
    const addOrMerge = (sim: Sim) => {
      ...
      const v =
        ...
        (banner.maxPity + 1) *
          (sim.const +
            1 +
            (banner.maxConst + 2) * (+sim.guaranteed + 2 * sim.guaranteedPity));
